interface CVSkillsProps {
  data: {
    ...
    languages: Array<{ name: string; level: string; percent: number }>
  }
}
        ...
        {categories.map(cat => {
          ...
          return skills.length > 0 ? (
            <div key={cat.key} className="cv-skills__category">
              <h3 className="cv-skills__category-title">{cat.label}</h3>
          ...
          {data.soft.map(skill => (
            <div key={skill.name} className="soft-skill">
              ...
              <div className="soft-skill__level">
                {[...Array(5)].map((_, i) => (
                  <span key={i} className={`soft-skill__dot ${i < Math.round(skill.level / 20) ? 'filled' : ''}`} />
                ))}
              </div>
            </div>
          ))}
