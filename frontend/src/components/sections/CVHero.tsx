...
export function CVHero({ data }: CVHeroProps) {
  ...
  return (
    <header className="cv-hero">
      ...
      <h1 className="cv-hero__name">{data.name}</h1>
      <p className="cv-hero__title">{data.title}</p>
      <p className="cv-hero__tagline">{data.tagline}</p>

      <div className="cv-hero__contacts">
        <a href={`mailto:${data.email}`} className="contact-item">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="M22 6L12 13L2 6" /></svg>
          {data.email}
        </a>
        ...
        <a href={`https://${data.website}`} target="_blank" rel="noopener noreferrer" className="contact-item">
      ...
      <div className="cv-hero__social">
        <a href={`https://${data.linkedin}`} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
          <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
        </a>
        <a href={`https://${data.github}`} target="_blank" rel="noopener noreferrer" aria-label="GitHub">
